...
function getFuroLoginURL() {
  if (!FuroClientId) throw new Error("ClientId needed to get the Furo login url");

  const baseUrl = `${AUTH_DOMAIN}/login/${FuroClientId}`;
  if (FuroRedirectUri) return `${baseUrl}?redirect_uri=${encodeURIComponent(FuroRedirectUri)}`;
  else return baseUrl;
}
